...
10. Spawn a Librarian Committee agent to record epistemic assertions for this task.
    Use the Task tool with subagent_type="general-purpose" and this prompt:

    ---
    You are the Librarian Committee agent for SEFI@Home.
    Your job: append epistemic assertions for $task_id to docs/assertions.md.

    Read these files:
    - ~/.claude/skills/librarian/SKILL.md  (epistemic review rubric)
    - $REPO_DIR/.claude/skills/librarian/SKILL.md  (project context)
    - $task_file  (acceptance criteria and what was implemented)
    - All test files under $REPO_DIR/tests/ that relate to this task

    For each module touched by $task_id, write one assertion block using this format:

    ## Assertion: $task_id — <module name>
    - **Agent:** Librarian-Committee
    - **Claim:** "<one-sentence claim about what this module does correctly>"
    - **Evidence:** <test file and line count, or doc file>
    - **Evidence score:** X/3  (0=none, 1=partial, 2=tests exist, 3=comprehensive)
    - **Falsifiability:** X/3
    - **Scope:** X/3
    - **Clarity:** X/3
    - **Verdict:** ACCEPT | CONDITIONAL ACCEPT | NEEDS EVIDENCE
    - **HITL flag:** <Yes — reason | No>

    Append the block(s) to $REPO_DIR/docs/assertions.md (create the file if missing,
    with a "# Epistemic Assertions" header and a "Generated by Librarian Committee"
    note). Do NOT overwrite existing content — append only.

    Then: git -C $REPO_DIR add docs/assertions.md
          git -C $REPO_DIR commit -m "docs: Librarian Committee assertions for $task_id"
          git -C $REPO_DIR push

    Print: LIBRARIAN COMMITTEE DONE: $task_id
    ---

11. Print: COMMITTER DONE: $task_id
PROMPT
}
